...
- [x] `SidecarRuntimeConfig` is an exact frozen, slot-only value object with
...
- [x] `SidecarRuntimeConfig` and `prepare_sidecar_runtime_config` are identical
...
- [x] `project_root` accepts only an exact built-in `str` or exact platform
  ...
  identity.
- [x] `project_id` is exactly `"project-v1-" +
...
- [x] Production captures `platformdirs.user_runtime_path` at import and calls
...
- [x] The platformdirs result must be an exact platform `Path` that is already
...
- [x] The exact platformdirs `Path` is passed to one canonical P0-001
  ...
  provenance boundary.
- [x] `requested_port` accepts only `None` or an exact built-in `int` in
...
- [x] `startup_timeout` accepts only exact built-in `int`/`float` values that
...
- [x] Success contains only exact scalar values, is value-equal for equal
...
- [x] A `KeyboardInterrupt`, `SystemExit`, or other non-`Exception`
...
- [x] Deterministic tests cover public shape, direct construction, frozen/slots,
...
- [x] Static and fault-injection evidence proves production performs only exact
...
- Reviewer 4: test adversary found a keyword-`self` constructor hole, weak
  ...
  P0/P1/P2 = 0 and GO.
- Reviewer 5: final test-only hardening review covered strict realpath dispatch,
  malformed fsencoded values, exact-boolean collaborator outputs, value-field
  equality, constant/body AST locks, and package export evidence. Focused 299
  and Phase 0 1,878 passed; final P0/P1/P2 = 0 and GO.
...
- Command: focused runtime-config tests; `make test-phase0`; `make check`;
  `make gate-phase0`
- Result: passed
- Notes: final focused tests passed 299/299 and Phase 0 tests passed
  1,878/1,878 on local CPython 3.13.5. The implementation candidate's local
  formatting, lint, strict production typing, agent-system checks, full
  repository suite (1,993/1,993), and sustained Phase 0 gate passed. Candidate
  `6ae39089625b39702b0bd6d4baa8f57eb499bdfc` passed
  [run 29163630397](https://github.com/alovwang-sys/FlowSight/actions/runs/29163630397)
  on its first attempt across Ubuntu/macOS and CPython 3.12/3.13. Final
  test-hardening candidate `90f7a0cecad72f5c3061b0b45f520e9dcec96496`
  passed
