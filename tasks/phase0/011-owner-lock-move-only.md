...
- [ ] Exact `OwnerLock.__copy__(self) -> NoReturn`,
...
- [ ] Copy/deepcopy/pickle failures perform zero `fileno`, flock, open, close,
...
- [ ] Exact AST evidence fixes the complete existing `OwnerLock` method set plus
...
- Command: pending
- Result: pending
- Notes: proves Python object duplication is blocked only; it does not prove
